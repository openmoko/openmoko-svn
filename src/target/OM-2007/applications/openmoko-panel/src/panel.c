...
#define PANEL_HIDDEN_SIZE 6 
  ...
  if (d->is_hidden)
	...
	/* Added By STEVEN */
		if(d->is_finger_mode)
			{
				if(panel_orig_size != 48)
					panel_orig_size = 48;
			}
		else
			{
				if(panel_orig_size != 32)
					panel_orig_size = 32;
			}
	/* Ended Added */
...
panel_set_bg(MBPanel *panel, int bg_type, char *bg_spec)
{
  ...
  switch (bg_type)
	...
	XFreePixmap(panel->dpy, panel->bg_pxm);
      ...
      break;
    case BG_SOLID_COLOR:
    	fprintf(stdout, "matchbox-panel: failed to load %s\n", bg_spec);
...
set_moko_type(MBPanel *panel)
{
  ...
  fprintf(stdout, "steven-panel: ATOM_MOKO_TYPE=[%s]\n", value);
  
  if(!strcmp(value,"style") || !strcmp(value, "style"))
  	panel->is_finger_mode = False;
  else 
  	panel->is_finger_mode = True;
...
panel_handle_property_notify(MBPanel *panel, XPropertyEvent *e)
{
  ...
  if (e->atom == panel->atoms[ATOM_MOKO_TYPE])
    {
      set_moko_type(panel);
    }
...
*panel_init(int argc, char *argv[])
{
  ...
  */
  ...
  panel->atoms[ATOM_MOKO_TYPE]
    = XInternAtom(panel->dpy, "_MOKO_TYPE_NAME", False);   
