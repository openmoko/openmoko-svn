                        ...
                        GdkRectangle    *allocation)
{
  ...
  if(allocation->width < FIC_WINDOW_MIN_SIZE_WIDTH)
  {
    allocation->width = FIC_WINDOW_MIN_SIZE_WIDTH;
  }

  /** if height small then minimum size, set the height to minimum size */
  if(allocation->height < FIC_WINDOW_MIN_SIZE_HEIGHT)
  {
    allocation->height = FIC_WINDOW_MIN_SIZE_HEIGHT;
  }
  ...
  fixed1 = lookup_widget (window, "fixed1");
  hboxtop = lookup_widget (window, "hboxtop");
  vboxpackage = lookup_widget (window, "vboxpackage");
  vboxtoolbar = lookup_widget (window, "vboxtoolbar");
  vboxsplit = lookup_widget (window, "vboxsplit");
  vboxdetailarea = lookup_widget (window, "vboxdetailarea");
  swbg = lookup_widget (window, "scrolledwindow1");
