...
init_depend_info_view (GtkWidget *window)
{
  ...
  depview = lookup_widget (window, STRING_TREE_VIEW_DEPEND);
  if (depview == NULL)
...
update_depend_info_view (GtkWidget *window, PACKAGE_DETAIL_INFO *detail)
{
  ...
  model = gtk_tree_view_get_model (GTK_TREE_VIEW (depview));

  if (model == NULL)
    {
      ERROR("Can\'t find the dependency view widget\n");
      return;
    }

  g_object_ref (model);

  gtk_tree_view_set_model (GTK_TREE_VIEW (depview), NULL);
  ...
  if (detail->depends != NULL)
    {
      len = strlen (detail->depends);
      pkg = g_malloc (len + 1);
      strcpy (pkg, detail->depends);
      pkg[len] = 0;
  ...
  gtk_tree_view_set_model (GTK_TREE_VIEW (depview), model);
  g_object_unref (model);
