...
mbdesktop_item_get_from_coords(MBDesktop *mb, int x, int y)
{
  ...
  /* Added By Steven*/
  /* 判断点中的ITEM是最左下角的回退图标否？*/
  /* 如果是TOP_LEVEL层的回退，不用处理 */

  /*fprintf(stdout, "steven-desktop: x =[%d] y =[%d] workarea_x =[%d] workarea_y =[%d] icon_size =[%d] workarea_weight =[%d] workarea_height =[%d]\n", x, y, mb->workarea_x, mb->workarea_y, mb->icon_size, mb->workarea_width, mb->workarea_height);*/
  

	  if( x > mb->workarea_x && x < mb->workarea_x + 76
	  	&& y > mb->workarea_height - 76 + mb->workarea_y && y < mb->workarea_height + mb->workarea_y)
	  {
	
	    /*fprintf(stdout,"steven-desktop: mb->scroll_offset_item = [%d]\n", mb->scroll_offset_item->type);	*/
	    	/*fprintf(stdout,"item->type = [%d]\n", item->type);*/
	    	/* mb->scroll_offset_item->type == ITEM_TYPE_PREVIOUS 表示刚进入一个FOLDER，
	    	   否则表示在这个FOLDER中已经scroll up or down过了
	    	   */
	    	if(mb->scroll_offset_item->type == ITEM_TYPE_PREVIOUS ||
	    		 ...
	    		 mb->scroll_offset_item->type == ITEM_TYPE_DOTDESKTOP_FOLDER  ) {
					...
					if(mb->scroll_offset_item->type == ITEM_TYPE_PREVIOUS)
						item = mb->scroll_offset_item;
					else
						item = mb->current_head_item->item_parent->item_child;
						
					/*fprintf(stdout,"steven-desktop: item->type = [%d]\n", item->type);	*/
	    		 	
	    		item->x = mb->workarea_x;
	    		item->y = mb->workarea_height - mb->icon_size;
	    		item->width = mb->icon_size;
